...
class DialogHelper {
    ...
    public async exportKeys(polygonOnly: boolean, detailed: boolean): Promise<void> {
        const keys = await this.inventoryHelper.getKeysInfo()

        if (polygonOnly && !this.hasDrawnPolygons()) {
        ...
        for (const info of keys.values()) {
            if (polygonOnly && !this.isInsideDrawnPolygon(info.portal.lat, info.portal.lng)) {
                continue
            }
        ...
        if (polygonOnly && exportData.length === 0) {
